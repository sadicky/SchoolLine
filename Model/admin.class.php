...
class Admin
{
    ...
    public function setAdmin($login, $pwd, $nom, $prenom, $tel, $email, $access)
    {
        $this->nom = $nom;
        $this->prenom = $prenom;
        $this->tel = $tel;
        ...
        $this->email = $email;
        $db = getconnection();
        $add = $db->prepare("INSERT INTO admin(MATRICULE,PWD,NOM,PRENOM,TEL,EMAIL,ACCESS)
        VALUES(?,?,?,?,?,?,?)");
        $addline = $add->execute(array($login, $pwd, $nom, $prenom, $tel, $email, $access)) or die(print_r($add->errorInfo()));
        return $addline;
    }
